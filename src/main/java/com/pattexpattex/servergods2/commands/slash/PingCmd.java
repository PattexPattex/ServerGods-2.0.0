...
public class PingCmd extends BotSlash {
    ...
    public void run(@NotNull SlashCommandEvent event) {
        ...
        event.getHook().editOriginalEmbeds(FormatUtil.defaultEmbed("`loading...`").build())
                .queue((msg) -> msg.editMessageEmbeds(FormatUtil.defaultEmbed(Emotes.GEAR + " **My ping is:** `" + (System.currentTimeMillis() - a) + "ms` | Gateway ping: `" + event.getJDA().getGatewayPing() + "ms`").build()).queue());
    }
